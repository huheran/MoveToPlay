...
任务现在会按“准备数据、校验、状态模型训练/评估/导出、事件模型训练/评估/导出、质量门禁、云端固件编译与打包”汇报进度。每次训练通过后，worker 使用固定版本的官方 ESP-IDF 镜像把 C 数组集成进 Dongle 固件，输出带 SHA-256 清单的完整烧录包。已批准任务组成模型版本库，可让云端重新编译固件或由电脑端下载烧录回滚；配置 `MOVETOPLAY_OSS_*` 与 `OSS_ACCESS_KEY_*` 后，批准动作会异步把完整模型目录压缩备份到私有 OSS Bucket。
