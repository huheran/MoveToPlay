...
任务现在会按“准备数据、校验、状态模型训练/评估/导出、事件模型训练/评估/导出、质量门禁、云端固件编译与打包”汇报进度。每次训练通过后，worker 使用固定 SHA-256 的乐鑫官方 ESP-IDF 5.5.2 源码和 ESP32-S3 工具链，把 C 数组集成进 Dongle 固件，输出带 SHA-256 清单的完整烧录包。已批准任务组成模型版本库，可让云端重新编译固件或由电脑端下载烧录回滚；模型支持独立的可读名称，同时保留不可变的技术版本号，官方完整数据训练出的首个批准模型会标记为“官方基线”。配置 `MOVETOPLAY_OSS_*` 与 `OSS_ACCESS_KEY_*` 后，批准动作会异步把完整模型目录压缩备份到私有 OSS Bucket。
